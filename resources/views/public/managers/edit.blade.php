...
@section('content')
    ...
    <section>
        ...
        <h1>Редактирование менеджера {{ $manager->name }}</h1>
            ...
            <form action="{{ route('managers.update', $manager->id) }}" method="post">
                ...
                <div class="flex gap-3 mb-4">
                    <div>
                        ...
                        <input type="text" id="name" name="name" value="{{ $manager->name }}" class="border-gray-300/50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-[#13A2AF]/30 focus:border-[#13A2AF]/30 block w-full p-2.5 outline-none"/>
                        @error('name')
                    ...
                    <div>
                        ...
                        <input type="text" id="id_billing" name="id_billing" value="{{ $manager->id_billing }}" class="border-gray-300/50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-[#13A2AF]/30 focus:border-[#13A2AF]/30 block w-full p-2.5 outline-none"/>
                        @error('id_billing')
                        <div class="text-xs text-red-500">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" id="id_crm" name="id_crm" value="{{ $manager->id_crm }}" class="border-gray-300/50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-[#13A2AF]/30 focus:border-[#13A2AF]/30 block w-full p-2.5 outline-none"/>
                        @error('id_crm')
                        <div class="text-xs text-red-500">{{ $message }}</div>
                        @enderror
                    </div>
