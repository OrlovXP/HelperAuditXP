...
@section('content')
    ...
    <section>
        ...
        <x-form.table-wrapper>
            ...
            <form action="{{ route('import') }}" method="POST" enctype="multipart/form-data" class="max-w-lg">
                @csrf

                <div class="w-full flex justify-between border border-gray-300/50 p-1 rounded-xl">
                    <input type="file" name="file">
                    <button type="submit" class="text-white bg-[#FF9119] hover:bg-[#FF9119]/80 focus:ring-4 focus:outline-none focus:ring-[#FF9119]/50 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center">
                        Импорт
                    </button>
                </div>
            </form>
            ...
            @if (!$categories->isEmpty())
                <x-form.table>
                    ...
                    <tr class="border-b">
                        ...
                        <x-table.th scope="col">Сумма по продукту</x-table.th>
                        <x-table.th scope="col">Вознаграждения</x-table.th>
                        <x-table.th scope="col">Кол-во сделок</x-table.th>
                        <x-table.th scope="col">L-сделок</x-table.th>
                        <x-table.th scope="col">S-сделок</x-table.th>
                        <x-table.th scope="col">D-сделок</x-table.th>

                        <x-table.th scope="col" class="text-green-500 border-r border-l">Полное</x-table.th>
                        <x-table.th scope="col" class="text-orange-400 border-r border-l">Частичное</x-table.th>

                        <x-table.th scope="col" class="text-red-500 border-r border-l">Всего</x-table.th>
                        <x-table.th scope="col" class="text-red-500 border-r border-l">L-сделки</x-table.th>
                        <x-table.th scope="col" class="text-red-500 border-r border-l">S-сделки</x-table.th>
                        <x-table.th scope="col" class="text-red-500 border-r border-l">D-сделки</x-table.th>
                    ...
                    @foreach($categories as $category)
                        <tr class="border-b border-gray-300/50 hover:bg-[#13A2AF]/10">
                            <x-table.td>
                                <a class="text-[#13A2AF]"
                            ...
                            <x-table.td>{{ $category->total_deals }}</x-table.td>
                            <x-table.td> {{ $category->total_l_deals }}</x-table.td>
                            <x-table.td> {{ $category->total_s_deals }}</x-table.td>
                            <x-table.td> {{ $category->total_d_deals }}</x-table.td>

                            <x-table.td class="text-green-500 border-r border-l">0</x-table.td>
                            <x-table.td class="text-orange-400 border-r border-l">0</x-table.td>

                            <x-table.td class="text-red-500 border-r border-l">0</x-table.td>
                            <x-table.td class="text-red-500 border-r border-l">0</x-table.td>
                            <x-table.td class="text-red-500 border-r border-l">0</x-table.td>
                            <x-table.td class="text-red-500 border-r border-l">0</x-table.td>
                            <x-table.td class="text-red-500">
                                <button id="dropdownMenuIconButton" data-dropdown-toggle="dropdown-{{ $category->id }}"
                                ...
                                <div id="dropdown-{{ $category->id }}" class="z-10 hidden">
                                    <ul class="text-sm flex flex-col gap-1" aria-labelledby="dropdownMenuIconButton">


                                        <li>
                                            <form action="{{ route('report-categories.destroy', ['id' => $category->id]) }}"
                                                  method="POST">
                                                @csrf
                                                @method('DELETE')
